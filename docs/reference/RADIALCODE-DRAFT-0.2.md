# RadialCode Design Draft 0.2

**Status:** especificação experimental de interoperabilidade  
**Autor:** RadialCode contributors  
**Licença deste documento:** CC BY 4.0  
**Implementação de referência:** RadialCode `0.1.0a1`

## 1. Escopo

Este documento define a geometria, o bootstrap, o frame, a proteção de canal, o alfabeto visual, a renderização e o fluxo mínimo de decode do **RadialCode Design Draft 0.2**. Uma implementação é interoperável quando produz os mesmos streams intermediários dos golden vectors e recupera seus payloads sem depender de metadados externos.
...
| Versão de formato | `1` |
| Versão deste documento | Design Draft 0.2 |
...
| Clock track | `0,22 R` | `0,25 R` | fase e sincronização angular |
...
A clock track tem 128 slots. O slot zero é um marcador de fase escuro. Os 127 slots restantes recebem uma m-sequence gerada por LFSR de sete bits, estado inicial `1111111₂` e polinômio `x⁷ + x⁶ + 1`.

A cada passo, o bit mais significativo é emitido, o feedback é o XOR dos bits 6 e 5, e o registrador desloca em direção ao bit mais significativo. O período DEVE ser 127.
...
Os vetores em `vectors/draft-0.2/manifest.json` congelam payload, frame, header, streams RS, interleaving, símbolos, máscara, SVG/PNG e hashes SHA-256. Uma implementação que reivindique conformidade com Draft 0.2 DEVE reproduzir os streams binários intermediários; diferenças apenas de rasterização são permitidas quando o SVG lógico e o decode permanecem equivalentes.
